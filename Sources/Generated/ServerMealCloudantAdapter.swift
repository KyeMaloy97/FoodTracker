...
public class ServerMealCloudantAdapter: ServerMealAdapter {
    let client: CouchDBClient
    var database: Database {
        // TODO: Properly ensure database exists
        client.createDB("servermeal") { _, _ in }
        return client.database("servermeal")
    }
    ...
    public func findAll(onCompletion: @escaping ([ServerMeal], Swift.Error?) -> Void) {
        database.retrieveAll(includeDocuments: true) { maybeDocuments, error in
            ...
            do {
                let models = try documents.map { _, document -> ServerMeal in
                    ...
                    let model = try ServerMeal(json: modelJSON)
    ...
    public func create(_ model: ServerMeal, onCompletion: @escaping (ServerMeal?, Swift.Error?) -> Void) {
    ...
    public func deleteAll(onCompletion: @escaping (Swift.Error?) -> Void) {
        client.deleteDB("servermeal") { error in
    ...
    public func findOne(_ maybeID: String?, onCompletion: @escaping (ServerMeal?, Swift.Error?) -> Void) {
    ...
    public func update(_ maybeID: String?, with model: ServerMeal, onCompletion: @escaping (ServerMeal?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
    ...
    public func delete(_ maybeID: String?, onCompletion: @escaping (ServerMeal?, Swift.Error?) -> Void) {
        guard let id = maybeID else {
            return onCompletion(nil, AdapterError.invalidId(maybeID))
        }
        database.retrieve(id) { maybeDocument, error in
